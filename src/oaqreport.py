...
#Generating the Outlier Queries
def random_oaquery_sample(max_dim):
    width = np.random.randint(1,max_dim)
    rn1 = (0,max_dim)
...
sample_num = 1000
max_dim = 16
range_list = [random_oaquery_sample(max_dim) for i in range(sample_num)]
...
while len(range_list)<sample_num:
    ...
    for i in range(sample_diff):
        range_list.append( random_oaquery_sample(max_dim) )
