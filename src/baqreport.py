...
def random_baquery_sample(max_dim):
    width = np.random.choice(range(1,max_dim+1), 2, replace=False)
    rn1 = random_range_calc(width[0], max_dim)
    rn2 = random_range_calc(width[0], max_dim)
    rn3 = random_range_calc(width[1], max_dim)
    rn4 = random_range_calc(width[1], max_dim)
...
sample_num = 1000
max_dim = 16
range_list = [random_baquery_sample(max_dim) for i in range(sample_num)]
...
while len(range_list)<sample_num:
    ...
    for i in range(sample_diff):
        range_list.append( random_baquery_sample(max_dim) )
